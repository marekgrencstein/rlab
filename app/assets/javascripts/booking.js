...
var book_start_time = 0;
var book_start_year = 0;
var book_start_month = 0;
var book_start_day   = 0;
...
function BOOKINGS()
{
	for( i=0; i < jasonbookings.length; i++ )
	{
		...
		var dayOffset  = start_date.getDate() - book_start_day;
		
		startTime = parseInt( (dayOffset*60*24 + start_date.getMinutes() + (start_date.getHours())*60 + timezone_offset) / time_step );

		book_times_start.push( startTime );
		book_times_steps.push( parseInt(book.step) + 1);
